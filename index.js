...
async function startBot() {
  ...
  // Check if sessionID is provided and process VOID-X-MD! format session
  if (config.sessionID && config.sessionID.startsWith('VOID-X-MD!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if (header !== 'VOID-X-MD' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'VOID-X-MD!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from VOID-X-MD Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing VOID-X-MD session:', e.message);
